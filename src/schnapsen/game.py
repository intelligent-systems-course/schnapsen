...
class Hand(CardCollection):
    ...
    def __init__(self, cards: Iterable[Card], max_size: int = 5) -> None:
        ...
        cards = list(cards)
        assert len(
            cards) <= max_size, f"The number of cards {len(cards)} is larger than the maximum number fo allowed cards {max_size}"
        self.cards = cards
...
class BotState:
    ...
    def get_move(self, player_perspective: 'PlayerPerspective', leader_move: Optional[Move]) -> Move:
        ...
        :param player_perspective: The PlayerPerspective which contains the information on the current state of the game from the perspective of this player
        :returns: The move the both played
        """
        move = self.implementation.get_move(player_perspective, leader_move=leader_move)
...
class PlayerPerspective(ABC):
    ...
    def make_assumption(self, rand: Random) -> GameState:
        ...
        assert len(unseen_talon) + len(unseen_opponent_hand) == len(
            unseen_cards), "Logical error. The number of unseen cards in the opponents hand and in the talon must be equal to the number of unseen cards"
...
class SchnapsenTrickImplementer(TrickImplementer):
    ...
    def play_trick(self, game_engine: 'GamePlayEngine', old_game_state: GameState) -> GameState:
        ...
        return self.play_trick_with_fixed_leader_move(game_engine=game_engine, old_game_state=old_game_state,
    ...
    def _apply_regular_trick(self, game_engine: 'GamePlayEngine', old_game_state: GameState,
        ...
        next_game_state.leader, next_game_state.follower, leader_remained_leader = game_engine.trick_scorer.score(trick,
    ...
    def get_leader_move(self, game_engine: 'GamePlayEngine', old_game_state: 'GameState') -> Move:
        ...
        leader_perspective = LeaderPerspective(old_game_state, game_engine)
        leader_move = game_engine.move_requester.get_move(old_game_state.leader, leader_perspective, None)
...
class MoveRequester:
    ...
    @abstractmethod
    def get_move(self, bot: BotState, player_perspective: PlayerPerspective, leader_move: Optional[Move]) -> Move:
        pass
...
class SimpleMoveRequester(MoveRequester):
    ...
    def get_move(self, bot: BotState, player_perspective: PlayerPerspective, leader_move: Optional[Move]) -> Move:
        return bot.get_move(player_perspective, leader_move=leader_move)
    ...
    def play_game_from_state(self, game_state: GameState, leader_move: Optional[Move]) -> Tuple[Bot, int, Score]:
        ...
        while not winner:
            if leader_move is not None:
                ...
                game_state = self.trick_implementer.play_trick_with_fixed_leader_move(self, game_state=game_state,
                                                                                      leader_move=leader_move)
                leader_move = None
            else:
        ...
        winner.implementation.notify_game_end(won=True, player_perspective=winner_state)

        loser_state = LoserPerspective(game_state, self)
        game_state.follower.implementation.notify_game_end(False, player_perspective=loser_state)
